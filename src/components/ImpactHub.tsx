    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-10">
        <div className="max-w-xl">
          <h2 className="text-5xl font-medium text-zinc-900 tracking-normal leading-none">Impact Hub</h2>
        ...
        <div className="flex bg-white border border-zinc-100 p-2 rounded-full shadow-sm shrink-0">
          <button onClick={() => setActiveTab('content')} className={`flex items-center gap-3 px-8 py-4 rounded-full text-[11px] font-medium uppercase tracking-wide transition-all ${activeTab === 'content' ? 'bg-[#233DFF] text-white shadow-xl' : 'text-zinc-400 hover:text-zinc-600'}`}><DollarSign size={16} /> {isComms ? 'Content Studio' : 'Fundraising'}</button>
          <button onClick={() => setActiveTab('leaderboard')} className={`flex items-center gap-3 px-8 py-4 rounded-full text-[11px] font-medium uppercase tracking-wide transition-all ${activeTab === 'leaderboard' ? 'bg-[#233DFF] text-white shadow-xl' : 'text-zinc-400 hover:text-zinc-600'}`}><BarChart3 size={16} /> Leaderboard</button>
          <button onClick={() => setActiveTab('rewards')} className={`flex items-center gap-3 px-8 py-4 rounded-full text-[11px] font-medium uppercase tracking-wide transition-all ${activeTab === 'rewards' ? 'bg-[#233DFF] text-white shadow-xl' : 'text-zinc-400 hover:text-zinc-600'}`}><Gift size={16} /> Rewards</button>
        </div>
      </div>
  ...
  return (
    <div className={`grid grid-cols-1 ${isComms ? 'lg:grid-cols-2' : ''} gap-10`}>
      <div className="bg-white p-12 rounded-[56px] border border-zinc-100 shadow-sm">
        <h3 className="text-2xl font-medium text-zinc-900 tracking-normal">AI-Powered Fundraising</h3>
        ...
        <div className="mt-10 pt-10 border-t border-zinc-100">
          {!draftedEmail && !isDraftingEmail && <button onClick={handleDraftEmail} className="bg-zinc-900 text-white px-12 py-7 rounded-full font-medium text-lg shadow-2xl hover:scale-105 transition-all flex items-center justify-center gap-4 group"><Sparkles size={24} /> Draft My Email</button>}
          ...
          {draftedEmail && (
            <div className="space-y-6 animate-in fade-in">
               ...
               <div className="flex items-center gap-4"><button onClick={() => handleCopy(draftedEmail)} className="flex-1 py-4 bg-[#233DFF] text-white rounded-full font-medium text-[11px] uppercase tracking-wide flex items-center justify-center gap-3">{isCopied ? <><Check size={16} /> Copied!</> : <><Copy size={16} /> Copy Email</>}</button><button onClick={() => setDraftedEmail('')} className="py-4 px-8 border border-zinc-200 rounded-full font-medium text-xs uppercase">Start Over</button></div>
            </div>
          )}
        </div>
      </div>
      ...
      {isComms && (
         <div className="bg-white p-12 rounded-[56px] border border-zinc-100 shadow-sm">
            <h3 className="text-2xl font-medium text-zinc-900 tracking-normal">Social Media Generator</h3>
            ...
            <div className="mt-10 pt-10 border-t border-zinc-100">
              {!draftedPost && !isDraftingPost && (
                  <div className="space-y-4">
                     ...
                     <div className="flex gap-2 p-1 bg-zinc-100 rounded-full"><button onClick={() => setPlatform('Instagram')} className={`flex-1 py-2 text-xs font-medium rounded-full flex items-center justify-center gap-2 ${platform === 'Instagram' ? 'bg-white shadow' : ''}`}><Instagram size={14}/> Instagram</button><button onClick={() => setPlatform('LinkedIn')} className={`flex-1 py-2 text-xs font-medium rounded-full flex items-center justify-center gap-2 ${platform === 'LinkedIn' ? 'bg-white shadow' : ''}`}><Linkedin size={14}/> LinkedIn</button></div>
                     <button onClick={handleDraftPost} disabled={!postTopic} className="w-full bg-zinc-900 text-white px-12 py-5 rounded-full font-medium text-sm shadow-2xl flex items-center justify-center gap-4 group disabled:opacity-50"><Sparkles size={20} /> Draft Post</button>
                  </div>
              )}
              ...
              {draftedPost && (
                <div className="space-y-6 animate-in fade-in">
                   ...
                   <div className="flex items-center gap-4"><button onClick={() => handleCopy(draftedPost)} className="flex-1 py-4 bg-[#233DFF] text-white rounded-full font-medium text-[11px] uppercase tracking-wide flex items-center justify-center gap-3">{isCopied ? <><Check size={16} /> Copied!</> : <><Copy size={16} /> Copy Post</>}</button><button onClick={() => setDraftedPost('')} className="py-4 px-8 border border-zinc-200 rounded-full font-medium text-xs uppercase">Start Over</button></div>
                </div>
              )}
            </div>
         </div>
...
const LeaderboardPanel: React.FC<{user: Volunteer, userRank: number, leaderboardData: Volunteer[]}> = ({user, userRank, leaderboardData}) => (
  <div className="bg-white p-12 rounded-[56px] border border-zinc-100 shadow-sm">
    <h3 className="text-2xl font-medium text-zinc-900 tracking-normal mb-8">Community Leaderboard</h3>
    <div className="space-y-4">
      {leaderboardData.map((v, index) => (
        <div key={v.id} className={`p-6 rounded-3xl border flex items-center justify-between transition-all ${user.id === v.id ? 'bg-[#233DFF] text-white border-[#233DFF]/20 shadow-xl' : 'bg-zinc-50 border-zinc-100'}`}>
          <div className="flex items-center gap-6">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-medium ${user.id === v.id ? 'bg-white/20' : 'bg-zinc-200 text-zinc-500'}`}>
              {index + 1}
            </div>
            <div className={`font-medium ${user.id === v.id ? 'text-white' : 'text-zinc-900'}`}>{v.name}</div>
          </div>
          <div className={`text-xl font-medium ${user.id === v.id ? 'text-white' : 'text-zinc-900'}`}>{v.points} <span className="text-xs font-medium opacity-50">XP</span></div>
        </div>
      ))}
      {userRank > 10 && (
         <div className="p-6 rounded-3xl border-2 border-dashed border-zinc-200 flex items-center justify-between">
           <div className="flex items-center gap-6">
             <div className="w-12 h-12 rounded-full flex items-center justify-center text-lg font-medium bg-zinc-200 text-zinc-500">{userRank}</div>
             <div className="font-medium text-zinc-900">{user.name} (You)</div>
           </div>
           <div className="text-xl font-medium text-zinc-900">{user.points} <span className="text-xs font-medium opacity-50">XP</span></div>
         </div>
      )}
    </div>
  </div>
  ...
  return (
   <div className="space-y-10">
      ...
      <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 p-12 rounded-[56px] text-white">
        <h3 className="text-2xl font-medium tracking-normal mb-2">Follow Us & Earn XP</h3>
      ...
      {/* Rewards Store */}
      <div className="bg-white p-12 rounded-[56px] border border-zinc-100 shadow-sm">
        <h3 className="text-2xl font-medium text-zinc-900 tracking-normal mb-2">Rewards Store</h3>
          ...
          {APP_CONFIG.GAMIFICATION.rewards.map(reward => {
            ...
            return (
              <div key={reward.id} className={`p-8 rounded-3xl border text-center flex flex-col items-center transition-all ${canAfford ? 'bg-white' : 'bg-zinc-50 opacity-60'}`}>
                ...
                <p className="text-base font-medium text-zinc-900 flex-1">{reward.title}</p>
                <p className="text-2xl font-medium text-zinc-900 my-4">{reward.points} <span className="text-xs font-medium text-zinc-300">XP</span></p>
                <button disabled={!canAfford} className="w-full py-3 bg-zinc-900 text-white rounded-full font-medium text-[10px] uppercase tracking-wide disabled:opacity-30 disabled:cursor-not-allowed">
                  Redeem
                </button>
              </div>
            )
