    ...
    <div style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", background: '#0f0f0f', color: '#fff', minHeight: '100vh', WebkitFontSmoothing: 'antialiased' }}>
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '64px 24px 80px' }}>
        ...
        <div style={{ marginBottom: 40, textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(245,158,11,.12)', border: '1px solid rgba(245,158,11,.3)', borderRadius: 100, padding: '8px 16px', marginBottom: 20, fontSize: 12, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(245,158,11,.85)' }}>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {/* Organization Name */}
            <div>
              <label style={labelStyle}>Organization Name <span style={{ color: '#f59e0b' }}>*</span></label>
              <input
                type="text"
                value={orgName}
            ...
            {/* Contact Name */}
            <div>
              <label style={labelStyle}>Your Full Name <span style={{ color: '#f59e0b' }}>*</span></label>
              <input
                type="text"
                value={contactName}
            ...
            {/* Email */}
            <div>
              <label style={labelStyle}>Your Email <span style={{ color: '#f59e0b' }}>*</span></label>
              <input
                type="email"
                value={email}
            ...
            {/* Password */}
            <div>
              <label style={labelStyle}>Password <span style={{ color: '#f59e0b' }}>*</span></label>
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: loading ? 'rgba(245,158,11,.5)' : '#f59e0b', color: '#0f0f0f', border: 'none', borderRadius: 100, padding: '16px 36px', fontSize: 15, fontWeight: 700, letterSpacing: '.02em', cursor: loading ? 'not-allowed' : 'pointer', marginTop: 4, transition: 'opacity .2s' }}
            >
              {loading ? (
                <>
            ...
            <p style={{ textAlign: 'center', fontSize: 14, color: 'rgba(255,255,255,.35)', margin: 0 }}>
              ...
              <button
                type="button"
                onClick={() => { window.history.replaceState({}, '', '/'); onLogin(true); }}
                style={{ background: 'none', border: 'none', color: '#f59e0b', cursor: 'pointer', fontSize: 14, fontWeight: 700, textDecoration: 'underline', textUnderlineOffset: 3, padding: 0 }}
              >
                Sign in
              </button>
            </p>
