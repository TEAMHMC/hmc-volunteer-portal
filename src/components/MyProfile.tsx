      ...
      {showReminderBanner && (
        <div className="bg-amber-50 border border-amber-200 p-6 rounded-[32px] flex items-center justify-between animate-in slide-in-from-top">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="text-sm font-medium text-amber-900">Weekly Availability Reminder</h4>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2 bg-amber-600 text-white rounded-full font-medium text-xs hover:bg-amber-700 transition-colors"
            >
              Update Now
            </button>
            <button
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="flex items-center gap-8">
          <div className="relative group">
            <div className="w-32 h-32 rounded-[40px] bg-zinc-900 border-4 border-white shadow-2xl overflow-hidden flex items-center justify-center text-white text-4xl font-medium">
          ...
          <div>
            {isEditing ? (
              <input
                ...
                className="text-4xl font-medium text-zinc-900 tracking-normal bg-zinc-50 border border-zinc-200 rounded-2xl px-4 py-2 outline-none focus:bg-white transition-all w-full max-w-md"
                placeholder="Your Name"
              />
            ) : (
              <h2 className="text-4xl font-medium text-zinc-900 tracking-tight">{currentUser.name}</h2>
            )}
        ...
        <button onClick={() => isEditing ? handleSaveProfile() : setIsEditing(true)} className={`px-8 py-4 rounded-full font-medium text-[11px] uppercase tracking-wide transition-all flex items-center gap-3 border border-black shadow-xl ${isEditing ? 'bg-[#233DFF] text-white' : 'bg-white text-zinc-900 hover:bg-zinc-50'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-10">
          <div className="bg-white p-12 rounded-[56px] border border-zinc-100 shadow-sm">
             <h3 className="text-2xl font-medium text-zinc-900 tracking-normal mb-8">Profile Details</h3>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                   <label className="text-[10px] font-medium text-zinc-400 uppercase tracking-wide px-2">Contact</label>
                   {isEditing ? (
                     <div className="space-y-4">
                       <input type="email" value={profileData.email} onChange={e => setProfileData({...profileData, email: e.target.value})} className="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-6 py-4 font-medium text-sm outline-none focus:bg-white transition-all" placeholder="Email" />
                       <input type="tel" value={profileData.phone} onChange={e => setProfileData({...profileData, phone: formatPhoneNumber(e.target.value)})} className="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-6 py-4 font-medium text-sm outline-none focus:bg-white transition-all" placeholder="Phone" />
                     </div>
                   ) : (
                     <div className="space-y-4">
                       <p className="text-sm font-medium text-zinc-700 flex items-center gap-3"><Mail size={16} className="text-zinc-300"/> {profileData.email}</p>
                       <p className="text-sm font-medium text-zinc-700 flex items-center gap-3"><Phone size={16} className="text-zinc-300"/> {profileData.phone || 'N/A'}</p>
                     </div>
                   )}
                </div>
                 <div className="space-y-4">
                   <label className="text-[10px] font-medium text-zinc-400 uppercase tracking-wide px-2">Notifications</label>
                    {isEditing ? (
                        ...
                        <div className="space-y-2">
                            <p className="text-sm font-medium text-zinc-700 flex items-center gap-3">{profileData.notificationPrefs.emailAlerts ? <Check size={16} className="text-emerald-500"/> : <XCircle size={16} className="text-rose-500"/>} Email Alerts</p>
                            <p className="text-sm font-medium text-zinc-700 flex items-center gap-3">{profileData.notificationPrefs.smsAlerts ? <Check size={16} className="text-emerald-500"/> : <XCircle size={16} className="text-rose-500"/>} SMS Alerts</p>
                        </div>
                    )}
                </div>
             </div>
          ...
          <div className="bg-white p-12 rounded-[56px] border border-zinc-100 shadow-sm">
            <h3 className="text-2xl font-medium text-zinc-900 tracking-normal mb-8">Availability</h3>
            {isEditing ? (
                 <div className="space-y-6">
                    <div>
                        <label className="text-sm font-medium text-zinc-600 mb-3 block">Available Days & Hours</label>
                        <div className="grid grid-cols-4 sm:grid-cols-7 gap-3 mb-4">
                            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => <button key={day} type="button" onClick={() => toggleDay(day)} className={`py-4 rounded-xl text-[10px] font-medium border transition-all ${profileData.availDays?.includes(day) ? 'bg-zinc-900 text-white border-zinc-900' : 'bg-white border-zinc-200'}`}>{day}</button>)}
                        </div>
                          ...
                          <div className="space-y-3">
                            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].filter(d => profileData.availDays.includes(d)).map(day => (
                              <div key={day} className="flex items-center gap-3 bg-zinc-50 p-3 rounded-xl border border-zinc-100">
                                <span className="text-xs font-medium text-zinc-700 w-10 shrink-0">{day}</span>
                                <input type="time" value={profileData.dayTimeSlots[day]?.start || '09:00'} onChange={e => updateDayTime(day, 'start', e.target.value)} className="bg-white border border-zinc-200 rounded-lg px-3 py-2 text-sm font-medium text-zinc-700 w-32" />
                                <span className="text-xs text-zinc-400 font-medium">to</span>
                                <input type="time" value={profileData.dayTimeSlots[day]?.end || '17:00'} onChange={e => updateDayTime(day, 'end', e.target.value)} className="bg-white border border-zinc-200 rounded-lg px-3 py-2 text-sm font-medium text-zinc-700 w-32" />
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    <div>
                        <label className="text-sm font-medium text-zinc-600 mb-3 block">Timezone</label>
                        <select value={profileData.timezone} onChange={e => setProfileData(prev => ({ ...prev, timezone: e.target.value }))} className="w-full bg-zinc-50 border border-zinc-100 rounded-xl px-4 py-3 text-sm font-medium text-zinc-700">
                     ...
                     <div>
                        <label className="text-sm font-medium text-zinc-600 mb-3 block">Time Off / Unavailable Dates</label>
                         <div className="flex gap-2">
                            ...
                            <button onClick={addUnavailableDate} className="px-4 py-2 bg-zinc-800 text-white rounded-xl text-sm font-medium"><Plus size={16}/></button>
                         </div>
                         <div className="flex flex-wrap gap-2 mt-4">
                             {profileData.unavailableDates.map(date => (
                                 <span key={date} className="flex items-center gap-2 bg-zinc-100 text-zinc-700 text-xs font-medium px-3 py-1 rounded-full">{date} <button onClick={() => removeUnavailableDate(date)}><Trash2 size={12} /></button></span>
                             ))}
                         </div>
                    </div>
                </div>
                 ...
                 <div className="space-y-6">
                    <div>
                        <label className="text-[10px] font-medium text-zinc-400 uppercase tracking-wide px-2 mb-3 block">Weekly Schedule</label>
                        {profileData.availDays.length > 0 ? (
                          <div className="space-y-2">
                            {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].filter(d => profileData.availDays.includes(d)).map(day => {
                              ...
                              return (
                                <div key={day} className="flex items-center gap-4 p-3 bg-zinc-50 rounded-xl border border-zinc-100">
                                  <span className="text-xs font-medium text-zinc-900 w-10">{day}</span>
                        ...
                        {profileData.timezone && (
                          <p className="text-[10px] text-zinc-400 font-medium mt-3 flex items-center gap-1.5"><Globe size={12} /> {profileData.timezone.replace('_', ' ')}</p>
                        )}
                    </div>
                    <div>
                        <label className="text-[10px] font-medium text-zinc-400 uppercase tracking-wide px-2 mb-2 block">Upcoming Time Off</label>
                        <div className="flex flex-wrap gap-2">
                            {profileData.unavailableDates.length > 0 ? profileData.unavailableDates.map(date => <span key={date} className="px-3 py-1 bg-zinc-100 text-zinc-700 rounded-full font-medium text-xs">{date}</span>) : <p className="text-xs text-zinc-400 italic">No dates specified.</p>}
                        </div>
                    </div>
                 </div>
            )}
        ...
        <div className="space-y-10">
          <div className="bg-white p-12 rounded-[56px] border border-zinc-100 shadow-sm space-y-10">
            <h3 className="text-2xl font-medium text-zinc-900 tracking-normal">Legacy Badges</h3>
            <div className="grid grid-cols-1 gap-4">
              {currentUser.achievements.length === 0 ? <p className="text-zinc-400 text-sm font-medium italic">No badges earned yet.</p> : currentUser.achievements.map(ach => (
                <div key={ach.id} className="flex items-center gap-5 p-5 bg-zinc-50 rounded-[28px] border border-zinc-100/50">
                  ...
                  <div>
                    <p className="text-sm font-medium text-zinc-900">{ach.title}</p>
                    <p className="text-[10px] text-zinc-400 font-medium uppercase tracking-wide">{new Date(ach.dateEarned).toLocaleDateString()}</p>
                  </div>
                </div>
              ))}
            </div>
