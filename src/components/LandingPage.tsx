              ...
              <div className="space-y-4 md:space-y-6">
                ...
                {googleClientId && (
                  <div className="pb-6 border-b border-zinc-100 space-y-4">
                    {/* Overlay approach: invisible GoogleLogin iframe receives real clicks;
                        custom styled div sits on top with pointer-events:none */}
                    <div className="relative w-full" style={{ height: 48 }}>
                      {/* Real Google auth button — invisible but clickable */}
                      <div className="absolute inset-0 opacity-[0.01] overflow-hidden" style={{ borderRadius: 999 }}>
                        <GoogleLogin
                          onSuccess={async (credentialResponse) => {
                            if (credentialResponse.credential) {
                              setGoogleLoading(true);
                              try {
                                await handleGoogleSubmit({ credential: credentialResponse.credential });
                              } finally {
                      ...
                      {/* Visual button — pointer-events:none so clicks fall through to iframe */}
                      <div className="pointer-events-none absolute inset-0 flex items-center justify-center gap-3 bg-white border border-zinc-200 rounded-full shadow-sm" style={{ minHeight: 48 }}>
                        {googleLoading ? (
                          ...
                          <>
                            ...
                            <span className="font-bold text-sm text-zinc-700">Continue with Google</span>
                          </>
                        )}
                      </div>
                    </div>
