      ...
      <div className="space-y-12 animate-in fade-in duration-500 pb-32">
        <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-10">
          <div className="max-w-xl">
            <h2 className="text-5xl font-medium text-zinc-900 tracking-normal">Volunteer Directory</h2>
            <p className="text-zinc-500 mt-4 font-medium text-lg leading-relaxed">
              Authorized personnel management for <span className="text-zinc-900 font-medium">{volunteers.filter(v => v.applicationStatus !== 'pendingReview').length}</span> verified community contributors.
            </p>
          </div>
          
          <div className="flex flex-col md:flex-row items-center gap-4">
            <button onClick={() => setShowImportModal(true)} className="py-5 px-6 bg-white border border-zinc-100 rounded-full text-zinc-400 font-medium text-[11px] uppercase tracking-wide flex items-center gap-3"><UploadCloud size={16}/>Bulk Import</button>
            <div className="relative group w-full md:w-auto">
              ...
              <input 
                ...
                className="pl-16 pr-8 py-5 bg-white border border-zinc-100 rounded-[32px] text-sm w-full md:w-64 outline-none shadow-sm focus:border-zinc-300 transition-all font-medium placeholder:text-zinc-200" 
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4 items-center">
          <div className="flex bg-white border border-zinc-100 p-1.5 rounded-full shadow-sm w-fit">
              <button onClick={() => setViewMode('all')} className={`px-6 py-3 rounded-full text-[11px] font-medium uppercase tracking-wide ${viewMode === 'all' ? 'bg-[#233DFF] text-white shadow-lg' : 'text-zinc-400'}`}>Directory</button>
              <button onClick={() => setViewMode('applicants')} className={`px-6 py-3 rounded-full text-[11px] font-medium uppercase tracking-wide relative ${viewMode === 'applicants' ? 'bg-[#233DFF] text-white shadow-lg' : 'text-zinc-400'}`}>
          ...
          <div className="flex bg-white border border-zinc-100 p-1.5 rounded-full shadow-sm">
              <button onClick={() => setGroupFilter('all')} className={`px-4 py-2 rounded-full text-[10px] font-medium uppercase tracking-wide ${groupFilter === 'all' ? 'bg-zinc-900 text-white' : 'text-zinc-400'}`}>All</button>
              <button onClick={() => setGroupFilter('group')} className={`px-4 py-2 rounded-full text-[10px] font-medium uppercase tracking-wide relative ${groupFilter === 'group' ? 'bg-purple-600 text-white' : 'text-zinc-400'}`}>
                Groups {groupVolunteersCount > 0 && <span className="ml-1 text-purple-400">({groupVolunteersCount})</span>}
              </button>
              <button onClick={() => setGroupFilter('returning')} className={`px-4 py-2 rounded-full text-[10px] font-medium uppercase tracking-wide relative ${groupFilter === 'returning' ? 'bg-emerald-600 text-white' : 'text-zinc-400'}`}>
                Returning {returningVolunteersCount > 0 && <span className="ml-1 text-emerald-400">({returningVolunteersCount})</span>}
              </button>
              <button onClick={() => setGroupFilter('individual')} className={`px-4 py-2 rounded-full text-[10px] font-medium uppercase tracking-wide ${groupFilter === 'individual' ? 'bg-zinc-900 text-white' : 'text-zinc-400'}`}>Individual</button>
          </div>
        </div>
            ...
            >
               <div className="flex justify-between items-start mb-8 relative z-10">
                  <div className="flex items-center gap-5">
                     <div className="w-16 h-16 rounded-[24px] bg-zinc-900 text-white flex items-center justify-center font-medium text-xl shadow-xl overflow-hidden border border-black/10">
                        {v.avatarUrl ? <img src={v.avatarUrl} className="w-full h-full object-cover" /> : v.name.charAt(0)}
                     </div>
                     <div>
                        <h3 className="text-lg font-medium text-zinc-900 leading-tight group-hover:text-[#233DFF] transition-colors">{v.name}</h3>
                        <p className="text-[10px] font-medium text-zinc-300 uppercase tracking-[0.15em] mt-1">{v.applicationStatus === 'pendingReview' ? `Applied for: ${v.appliedRole}` : v.role}</p>
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {v.isReturningVolunteer && (
                            <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[8px] font-medium uppercase tracking-wider rounded-full">Returning</span>
                          )}
                          {v.isGroupVolunteer && (
                            <span className="px-2 py-0.5 bg-purple-100 text-purple-700 text-[8px] font-medium uppercase tracking-wider rounded-full" title={v.groupName}>{v.groupType || 'Group'}</span>
                          )}
                        </div>
                     </div>
                  </div>
               ...
               <div className="bg-zinc-50/50 p-6 rounded-[32px] border border-zinc-100/50 mb-8 relative z-10">
                  <p className="text-[9px] font-medium text-zinc-300 uppercase tracking-wide mb-4">Compliance Status</p>
               ...
               <div className="space-y-4 relative z-10">
                  <div className="flex justify-between items-center text-[10px] font-medium uppercase tracking-wide text-zinc-300 px-1">
               ...
               <div className="mt-10 pt-8 border-t border-zinc-50 flex items-center justify-between relative z-10">
                  <div>
                     <p className="text-[9px] font-medium text-zinc-300 uppercase tracking-wide mb-1">Hours Contributed</p>
                     <p className="text-2xl font-medium text-zinc-900 tracking-normal leading-none">{Math.floor(v.hoursContributed)} <span className="text-xs font-medium text-zinc-300">HRS</span></p>
                  </div>
          ...
          <button
            ...
            className="bg-white border-2 border-dashed border-zinc-100 rounded-[56px] p-12 flex flex-col items-center justify-center text-zinc-300 gap-6 hover:bg-zinc-50/50 hover:border-[#233DFF]/20 hover:text-[#233DFF] transition-all group">
             ...
             <div className="text-center">
               <p className="text-[11px] font-medium uppercase tracking-[0.2em]">Add Volunteer</p>
           ...
           <div className="bg-white max-w-4xl w-full rounded-[56px] shadow-2xl border border-zinc-100 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
              <header className="p-10 border-b border-zinc-100 flex items-center justify-between shrink-0">
                 <div className="flex items-center gap-6">
                    <div className="w-20 h-20 rounded-[28px] bg-zinc-900 text-white flex items-center justify-center font-medium text-2xl shadow-xl overflow-hidden">
                    ...
                    <div>
                       <h2 className="text-3xl font-medium text-zinc-900 tracking-tight">{selectedVolunteer.name}</h2>
                       <p className="text-zinc-400 font-medium uppercase tracking-wide text-xs mt-1">{selectedVolunteer.applicationStatus === 'pendingReview' ? `Applied for: ${selectedVolunteer.appliedRole}` : selectedVolunteer.role}</p>
                    </div>
                 </div>
              ...
              <main className="p-10 grid grid-cols-1 md:grid-cols-2 gap-10 overflow-y-auto">
                 <div className="space-y-8">
                    <div className="space-y-2">
                       <h4 className="text-xs font-medium text-zinc-400 uppercase tracking-wide">Contact Info</h4>
                       <div className="p-6 bg-zinc-50 rounded-2xl border border-zinc-100 space-y-3">
                          <p className="flex items-center gap-3 font-medium text-sm text-zinc-700"><Mail size={16} className="text-zinc-300"/> {selectedVolunteer.email}</p>
                          <p className="flex items-center gap-3 font-medium text-sm text-zinc-700"><Phone size={16} className="text-zinc-300"/> {selectedVolunteer.phone}</p>
                       </div>
                    </div>
                    ...
                    <div className="space-y-2">
                       <div className="flex justify-between items-center">
                         <h4 className="text-xs font-medium text-zinc-400 uppercase tracking-wide">Tags</h4>
                         <button onClick={() => setEditingTags(!editingTags)} className="text-xs font-medium">{editingTags ? 'Done' : 'Edit Tags'}</button>
                       </div>
                       <div className="p-6 bg-zinc-50 rounded-2xl border border-zinc-100">
                          <div className="flex flex-wrap gap-2">
                            {selectedVolunteer.tags?.map(t => (
                              <span key={t} className="flex items-center gap-1 px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-xs font-medium">
                                {t}
                          ...
                          {editingTags && (
                             <div className="flex gap-2 mt-4">
                                ...
                                <button onClick={handleAddTag} className="px-3 py-1 bg-zinc-800 text-white rounded-lg text-xs font-medium">Add</button>
                             </div>
                          )}
                       </div>
                    </div>
                     <div className="space-y-2">
                       <h4 className="text-xs font-medium text-zinc-400 uppercase tracking-wide">Achievements</h4>
                       <div className="space-y-2">
                          {selectedVolunteer.achievements.map(a => (
                            <div key={a.id} className="flex items-center gap-3 p-3 bg-zinc-50 rounded-xl border border-zinc-100">
                               ...
                               <p className="font-medium text-xs text-zinc-700">{a.title}</p>
                            </div>
                          ))}
                       </div>
                    </div>
                 ...
                 <div className="space-y-6">
                    <div className="bg-zinc-50/70 p-8 rounded-[32px] border border-zinc-100 space-y-6">
                      <h4 className="text-xs font-medium text-zinc-400 uppercase tracking-wide">Compliance Checklist</h4>
                      {Object.values(selectedVolunteer.compliance).map((c: ComplianceStep) => (
                        <div key={c.id} className="flex items-center gap-4">
                          ...
                          <p className={`font-medium text-sm ${c.status === 'verified' || c.status === 'completed' ? 'text-zinc-800' : 'text-zinc-400'}`}>{c.label}</p>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-[32px] border border-zinc-100 space-y-4">
                      <div className="flex items-center justify-between">
                        <h4 className="text-xs font-medium text-zinc-400 uppercase tracking-wide">Assigned Tasks</h4>
                        <button
                          onClick={() => setShowTaskModal(true)}
                          className="px-3 py-1.5 bg-[#233DFF] text-white rounded-lg text-xs font-medium flex items-center gap-1"
                        >
                          ...
                          {selectedVolunteer.tasks.map(task => (
                            <div key={task.id} className={`p-4 rounded-xl border ${task.status === 'completed' ? 'bg-emerald-50 border-emerald-100' : 'bg-zinc-50 border-zinc-100'}`}>
                              <div className="flex items-start justify-between">
                                <div>
                                  <p className={`font-medium text-sm ${task.status === 'completed' ? 'text-emerald-700 line-through' : 'text-zinc-800'}`}>{task.title}</p>
                                  <p className="text-xs text-zinc-500 mt-1">{task.description}</p>
                                </div>
                                <span className={`px-2 py-1 rounded-full text-[10px] font-medium uppercase ${task.status === 'completed' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                                  {task.status}
                                </span>
                              </div>
                              {task.dueDate && (
      ...
      {showTaskModal && selectedVolunteer && (
        <div className="fixed inset-0 bg-zinc-900/80 backdrop-blur-md z-[1100] flex items-center justify-center p-8 animate-in fade-in" onClick={() => setShowTaskModal(false)}>
          <div className="bg-white max-w-lg w-full rounded-[40px] shadow-2xl border border-zinc-100 p-10 space-y-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-medium text-zinc-900 tracking-tight">Assign Task</h2>
              <button onClick={() => setShowTaskModal(false)} className="p-2 bg-zinc-100 rounded-full text-zinc-400 hover:text-zinc-900">
                <X size={20} />
              </button>
            </div>
            <p className="text-zinc-500 text-sm">Assign a task to <span className="font-medium text-zinc-800">{selectedVolunteer.name}</span></p>

            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">Task Title *</label>
                <input
                  type="text"
                  value={taskTitle}
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">Due Date (Optional)</label>
                <input
                  type="date"
                  value={taskDueDate}
            ...
            <button
              ...
              className="w-full py-4 bg-[#233DFF] border border-black text-white rounded-full font-medium text-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform disabled:opacity-50"
            >
        ...
        <div className="fixed inset-0 bg-zinc-900/80 backdrop-blur-md z-[1200] flex items-center justify-center p-8 animate-in fade-in" onClick={() => setShowDeleteConfirm(false)}>
          <div className="bg-white max-w-md w-full rounded-3xl shadow-2xl border border-zinc-100 p-10 space-y-6" onClick={e => e.stopPropagation()}>
            <div className="text-center">
              ...
              <h2 className="text-xl font-medium text-zinc-900">Delete Volunteer?</h2>
              <p className="text-zinc-500 text-sm mt-2">
                This will permanently delete <span className="font-medium text-zinc-800">{selectedVolunteer.name}</span>'s account, remove them from all assigned shifts, and delete their login credentials. This cannot be undone.
              </p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-3 bg-zinc-100 text-zinc-700 rounded-xl font-medium text-sm">Cancel</button>
              <button onClick={handleDeleteVolunteer} disabled={isDeleting} className="flex-1 py-3 bg-rose-500 text-white rounded-xl font-medium text-sm flex items-center justify-center gap-2 hover:bg-rose-600 disabled:opacity-50">
...
const RoleManagementPanel: React.FC<{currentUser: Volunteer, selectedVolunteer: Volunteer, onUpdate: (v: Volunteer) => void}> = ({currentUser, selectedVolunteer, onUpdate}) => {
    ...
    return (
        <div className="space-y-2">
            <h4 className="text-xs font-medium text-zinc-400 uppercase tracking-wide">Role Management</h4>
            <div className="p-6 bg-zinc-50 rounded-2xl border border-zinc-100">
                {currentUser.isAdmin ? (
                    isEditingRole ? (
                        <div className="space-y-4">
                            <select value={newRole} onChange={e => setNewRole(e.target.value)} className="w-full p-3 bg-white border border-zinc-200 rounded-lg font-medium">
                                {APP_CONFIG.HMC_ROLES.map(r => <option key={r.id} value={r.label}>{r.label}</option>)}
                            </select>
                            <div className="flex gap-2">
                                <button onClick={() => setIsEditingRole(false)} className="flex-1 text-xs font-medium py-2 border rounded-lg">Cancel</button>
                                <button onClick={handleSaveRole} className="flex-1 text-xs font-medium py-2 bg-zinc-800 text-white rounded-lg">Save Role</button>
                            </div>
                        </div>
                    ) : (
                        <div className="flex items-center justify-between">
                            <p className="font-medium">{selectedVolunteer.role}</p>
                            <button onClick={() => setIsEditingRole(true)} className="text-xs font-medium">Edit Role</button>
                        </div>
                    )
                ) : currentUser.role === 'Volunteer Lead' ? (
                     <button onClick={handlePromote} className="w-full text-center py-3 bg-indigo-100 text-indigo-700 rounded-lg text-xs font-medium">
...
const ApplicationReviewPanel: React.FC<{volunteer: Volunteer, onReview: (action: 'approve' | 'reject', notes: string) => void, isReviewing: boolean}> = ({ volunteer, onReview, isReviewing }) => {
    ...
    return (
        <div className="md:col-span-2 bg-amber-50 p-6 rounded-[32px] border-2 border-amber-200 space-y-4 animate-in fade-in">
            <h3 className="text-sm font-medium text-amber-800 uppercase tracking-wide">Application Review for: {volunteer.appliedRole}</h3>
                ...
                <div className="space-y-3">
                    {volunteer.roleAssessment.map((qa, i) => (
                        <div key={i} className="bg-white/60 p-4 rounded-xl">
                            <p className="text-xs font-medium text-zinc-500">{qa.question}</p>
            ...
            <div className="flex items-center gap-4">
                <button onClick={() => onReview('approve', notes)} disabled={isReviewing} className="flex-1 py-4 bg-emerald-600 text-white rounded-full font-medium text-xs uppercase tracking-wide flex items-center justify-center gap-2 hover:bg-emerald-700 disabled:opacity-50">
                    {isReviewing ? <Loader2 size={16} className="animate-spin" /> : <><Check size={16}/> Approve for Role</>}
                </button>
                <button onClick={() => onReview('reject', notes)} disabled={isReviewing} className="flex-1 py-4 bg-indigo-600 text-white rounded-full font-medium text-xs uppercase tracking-wide flex items-center justify-center gap-2 hover:bg-indigo-700 disabled:opacity-50">
    ...
    return (
        <div className="fixed inset-0 bg-zinc-900/80 backdrop-blur-md z-[1000] flex items-center justify-center p-8 animate-in fade-in" onClick={onClose}>
            <div className="bg-white max-w-2xl w-full rounded-[56px] shadow-2xl border border-zinc-100 p-12 space-y-8" onClick={e => e.stopPropagation()}>
                <h2 className="text-3xl font-medium text-zinc-900 tracking-tight">Bulk Import Volunteers</h2>
                ...
                {successCount !== null ? (
                    ...
                    <>
                        ...
                        <button onClick={handleUpload} disabled={isUploading || !file} className="w-full py-5 bg-[#233DFF] text-white rounded-full font-medium text-sm uppercase tracking-wide flex items-center justify-center gap-4 disabled:opacity-50">
...
const AddVolunteerModal: React.FC<{onClose: () => void, setVolunteers: Function}> = ({ onClose, setVolunteers }) => {
        ...
        return (
            <div className="fixed inset-0 bg-zinc-900/80 backdrop-blur-md z-[1000] flex items-center justify-center p-8 animate-in fade-in">
                <div className="bg-white max-w-lg w-full rounded-[40px] shadow-2xl border border-zinc-100 p-12 text-center">
                    ...
                    <h3 className="text-2xl font-medium text-emerald-800">Volunteer Added!</h3>
    ...
    return (
        <div className="fixed inset-0 bg-zinc-900/80 backdrop-blur-md z-[1000] flex items-center justify-center p-8 animate-in fade-in" onClick={onClose}>
            <div className="bg-white max-w-lg w-full rounded-[40px] shadow-2xl border border-zinc-100 p-10 space-y-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-medium text-zinc-900 tracking-tight">Add Volunteer</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">First Name *</label>
                            <input
                                type="text"
                        ...
                        <div>
                            <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">Last Name *</label>
                            <input
                                type="text"
                    ...
                    <div>
                        <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">Email *</label>
                        <input
                            type="email"
                    ...
                    <div>
                        <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">Phone</label>
                        <input
                            type="tel"
                    ...
                    <div>
                        <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">Role</label>
                        <select
                            value={formData.role}
                    ...
                    <div className="pt-2 border-t border-zinc-100">
                        <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">Temporary Password</label>
                        <input
                            type="password"
                    ...
                    <button
                        type="submit"
                        disabled={isSaving}
                        className="w-full py-4 bg-[#233DFF] text-white rounded-full font-medium text-sm uppercase tracking-wide flex items-center justify-center gap-2 disabled:opacity-50"
                    >
