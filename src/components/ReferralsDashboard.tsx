...
const getSlaStatus = (referral: ReferralRecord): { status: ReferralRecord['slaComplianceStatus'], color: string } => {
    ...
    return { status: 'On Track', color: 'bg-blue-500' };
};
    ...
    return (
        <div className="space-y-12 animate-in fade-in duration-500">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <button onClick={() => setSelectedReferral('new')} className="flex items-center gap-3 px-6 py-4 bg-blue-600 text-white rounded-full text-xs font-black uppercase tracking-widest shadow-lg hover:bg-blue-700 transition-colors">
        ...
        <div className="fixed inset-0 bg-zinc-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-8" onClick={onClose}>
            <div className="bg-white max-w-4xl w-full rounded-[40px] shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <footer className="p-8 border-t border-zinc-100 flex justify-end">
                    <button onClick={handleSubmit} disabled={isSaving} className="flex items-center gap-3 px-6 py-3 bg-blue-600 text-white rounded-full text-xs font-black uppercase tracking-widest shadow-lg hover:bg-blue-700 disabled:opacity-50">
    ...
    return (
        <div className="p-4 bg-blue-50/50 rounded-2xl border border-blue-100 space-y-4">
            <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold text-blue-700 uppercase flex items-center gap-2"><Sparkles size={14}/> AI Matching Assistant</h4>
                <button onClick={handleFindMatch} disabled={!serviceNeed || isLoading} className="px-3 py-1 bg-white border border-blue-200 text-blue-700 text-xs font-bold rounded-lg disabled:opacity-50">
            ...
            {recommendations.length > 0 && (
                <div className="space-y-2">
                    {recommendations.map((rec, i) => {
                         ...
                         return (
                             <div key={i} className="p-3 bg-white/50 rounded-lg border border-blue-100">
                                 ...
                                 <button onClick={() => onSelect(resource)} className="text-xs font-bold text-blue-600 hover:underline">Select</button>
                             </div>
                         )
                    })}
                </div>
